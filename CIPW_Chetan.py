...
sinclas_results = pd.read_csv('DataOnlyChemical_res.csv',encoding='latin-1')
minerals = ['apatite','ilmenite','orthoclase','albite','anorthite','corundum',
           'titanite','magnetite','diopside','wollastonite','hypersthene','quartz','olivine',
           'nepheline','leucite', 'perovskite']

x = [sinclas_results['AP_NORM'],sinclas_results['IL_NORM'], sinclas_results['OR_NORM'],
     sinclas_results['AB_NORM'], sinclas_results['AN_NORM'], sinclas_results['C_NORM'],
     sinclas_results['SPH_NORM'], sinclas_results['MT_NORM'], sinclas_results['DI_NORM'],
     sinclas_results['WO_NORM'], sinclas_results['HY_NORM'], sinclas_results['Q_NORM'],
     sinclas_results['OL_NORM'],sinclas_results['OL_NORM'],sinclas_results['NE_NORM'], 
     sinclas_results['LC_NORM'], sinclas_results['PER_NORM']]
y = [norm['apatite'], norm['ilmenite'],norm['orthoclase'],norm['albite'], norm['anorthite'],
    norm['corundum'],norm['titanite'], norm['magnetite'], norm['diopside'], norm['wollastonite'],
    norm['hypersthene'],norm['quartz'], norm['olivine'], norm['leucite'], norm['nepheline'], norm['perovskite']]
...
for i in axs.flatten():
    if n < len(x):
        i.scatter(x[n],y[n])
        mask = ~np.isnan(x[n]) & ~np.isnan(y[n])
        linreg = scipy.stats.linregress(x[n][mask],y[n][mask])
        print(linreg)
        i.text(0, 1, linreg.rvalue)
        i.set_title(str(minerals[n]))
    n = n + 1

fig.tight_layout()
plt.savefig('CIPW_tests.png', dpi = 1200)
plt.show()
